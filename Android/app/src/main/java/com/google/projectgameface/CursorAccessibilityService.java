...
public class CursorAccessibilityService extends AccessibilityService implements LifecycleOwner {
    ...
    public static final String ACTION_CURSOR_POSITION_UPDATE = "CURSOR_POSITION_UPDATE";

    /** Limit UI update rate to 60 fps */
    public static final int UI_UPDATE = 16;
    private static final int CURSOR_POSITION_BROADCAST_INTERVAL_MS = 120;
    ...
    private long lastCursorPositionBroadcast = 0;
            ...
            public void run() {
                ...
                switch (serviceState) {
                    ...
                    case ENABLE:
                        ...
                        sendCursorPositionBroadcast(cursorController.getCursorPositionXY());
    ...
    private void sendCursorPositionBroadcast(int[] cursorPosition) {
        long now = SystemClock.uptimeMillis();
        if ((now - lastCursorPositionBroadcast) < CURSOR_POSITION_BROADCAST_INTERVAL_MS) {
            return;
        }

        Intent intent = new Intent(ACTION_CURSOR_POSITION_UPDATE);
        intent.putExtra("x", cursorPosition[0]);
        intent.putExtra("y", cursorPosition[1]);
        intent.putExtra("screenWidth", screenSize.x);
        intent.putExtra("screenHeight", screenSize.y);
        intent.putExtra("state", serviceState.ordinal());
        sendBroadcast(intent);
        lastCursorPositionBroadcast = now;
    }
